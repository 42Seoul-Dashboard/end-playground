...
let Api42sService = class Api42sService {
    ...
    async getApi42ByCol(no) {
        const found = await this.api42Repository.createQueryBuilder('api42').getMany();
        let temp;
        console.log(found);
        temp = [];
        for (let i = 0; i < 6; i++) {
            switch (no) {
                ...
                case "학적":
                    ...
                    break;
                default:
                    throw new common_1.NotFoundException(`Can't find Api42 with ${no}`);
                    break;
            }
        }
        return temp;
    ...
    async getAllApi42() {
        return await this.api42Repository.find();
    }
    async createApi42(createApi42Dto) {
        ...
        await axios({
            method: "get",
            url: `http://api42sheets.google.com/tq?key=${key_1.KEY}&pub=1`,
